...
from apps.compras.models import Compra,Insumo,Proveedor,DetalleCompra
...
from apps.compras.forms import CompraForm, DetalleCompraForm
...
class  Inicio(LoginRequiredMixin,TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        top_clientes = Venta.objects.values('cliente__nombre', 'cliente__apellidos') \
                            .annotate(total_ventas=Sum('total_venta')) \
                            .order_by('-total_ventas')[:5]
        top_clientes_json = json.dumps(list(top_clientes))

        context['top_clientes_json'] = top_clientes_json
...
class ResultadoAnalisisDetailView(DetailView):
    model = ResultadoAnalisis
    template_name = 'resultado.html'
    context_object_name = 'resultado_analisis'
    pk_url_kwarg = 'id_resultado'

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        resultado_analisis = self.get_object()
        
        # Obtener el contenido Markdown del campo 'response'
        markdown_content = resultado_analisis.resultado['result']['response']
        ...
        context['html_content'] = html_content
         ...
         # Obtener los datos del top de clientes
        top_clientes = Venta.objects.values('cliente__nombre', 'cliente__apellidos') \
                                     .annotate(total_ventas=Sum('total_venta')) \
                                     .order_by('-total_ventas')[:5]

        # Convertir los datos a una lista de diccionarios
        top_clientes_data = list(top_clientes)

        # Convertir los datos a JSON
        top_clientes_json = json.dumps(top_clientes_data)

        # Agregar el JSON al contexto
        context['top_clientes_json'] = top_clientes_json
         # Obtener los datos del top de insumos
        top_insumos = Venta.objects.values('insumos__nombre') \
                                    .annotate(total_usos=Count('insumos')) \
                                    .order_by('-total_usos')[:5]

        # Convertir los datos a una lista de diccionarios
        top_insumos_data = list(top_insumos)

        # Convertir los datos a JSON
        top_insumos_json = json.dumps(top_insumos_data)

        # Agregar el JSON al contexto
        context['top_insumos_json'] = top_insumos_json
        return context
...
def crear_compra(request):
    ...
    if request.method == 'POST':
        ...
        if compra_form.is_valid() and detalle_formset.is_valid():
            compra = compra_form.save()  # Guardar la compra primero
            detalle_formset.instance = compra  # Asignar la compra al detalle
            detalle_formset.save()  # Guardar los detalles de la compra
            ...
            return redirect('web:compras')  # Redirigir a la lista de compras
    else:
