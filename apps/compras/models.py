...
class Compra(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        suma_precios = sum(detalle.insumo.precio * detalle.cantidad for detalle in self.detalles.all())
        self.total_compra = suma_precios

        # Aplica impuestos si la compra está facturada
        if self.factura:
            impuesto_total = self.impuesto_iva + self.impuesto_it
            self.total_compra += (self.total_compra * impuesto_total) / 100
            ...
            factura = Factura(
                ...
                tipo='Compra',
                detalle=f'Detalle de la compra {self.id_compra}. Insumos: {", ".join(detalle.insumo.nombre for detalle in self.detalles.all())}',
            ...
            recibo = Recibo(
                tipo='Compra',
                detalle=f'Detalle del recibo de la compra {self.id_compra}. Insumos: {", ".join(detalle.insumo.nombre for detalle in self.detalles.all())}',
                total=self.total_compra
            )
            recibo.save()
        ...
        for detalle in self.detalles.all():
            stock, created = StockInsumo.objects.get_or_create(
                inventario=self.inventario,  # Cambiado a self.inventario
